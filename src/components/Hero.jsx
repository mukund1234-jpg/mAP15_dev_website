...
function useTyping(elRef) {
  useEffect(() => {
    const words = ["web applications", "REST APIs", "mobile apps", "design systems", "dev tools", "open source"];
    ...
    function type() {
      ...
      let d = del ? 55 : 105;
      if (!del && ci > w.length) { del = true; d = 1700; }
      if (del && ci < 0)  { del = false; wi = (wi + 1) % words.length; ci = 0; d = 280; }
...
function useThreeCanvas(canvasRef) {
  useEffect(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      vel.push({ x: (Math.random() - .5) * .035, y: (Math.random() - .5) * .022 });
    }
    const geo = new THREE.BufferGeometry();
    geo.setAttribute("position", new THREE.BufferAttribute(pos, 3));
    scene.add(new THREE.Points(geo, new THREE.PointsMaterial({ color: 0x63d2ff, size: .55, transparent: true, opacity: .45 })));
    ...
    scene.add(new THREE.LineSegments(lgeo, new THREE.LineBasicMaterial({ color: 0x63d2ff, transparent: true, opacity: .055 })));

    // Wireframe objects
    const addW = (g, col, x, y, z) => {
      const m = new THREE.Mesh(g, new THREE.MeshBasicMaterial({ color: col, wireframe: true, transparent: true, opacity: .09 }));
    ...
    const o3 = addW(new THREE.TorusGeometry(9, 1, 8, 20),0xff7b3d, -15,  28, -40);

    let cx2 = 0, cy2 = 0, cmx = 0, cmy = 0, fr = 0;
    const onMouse = (e) => {
      cmx = (e.clientX / innerWidth - .5) * 10;
      cmy = -(e.clientY / innerHeight - .5) * 7;
    };
    ...
    const animate = () => {
      ...
      if (fr % 3 === 0) {
        ...
        for (let i = 0; i < N && lc < 1800; i++)
          for (let j = i + 1; j < N && lc < 1800; j++) {
            ...
            if (Math.abs(dx) < D && Math.abs(dy) < D && dx*dx + dy*dy < D*D) {
              lp[lc*6]   = pos[i*3];   lp[lc*6+1] = pos[i*3+1]; lp[lc*6+2] = pos[i*3+2];
              lp[lc*6+3] = pos[j*3];   lp[lc*6+4] = pos[j*3+1]; lp[lc*6+5] = pos[j*3+2];
              lc++;
            }
          }
      ...
      const t = fr * .008;
      o1.rotation.x += .003; o1.rotation.y += .005; o1.position.y = 18 + Math.sin(t*.7)*3;
      o2.rotation.x += .004; o2.rotation.z += .003; o2.position.y = -14 + Math.cos(t*.5)*4;
      o3.rotation.x += .002; o3.rotation.y += .003; o3.position.y = 28 + Math.sin(t*.6)*2;
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-[2] max-w-6xl mx-auto px-8 pt-[120px] pb-20 w-full">
        ...
        >
          Mukund<br /><span className="text-cyan">Aparaj.</span>
        </h1>
        ...
        <p className="h-bio max-w-[500px] text-muted text-base leading-[1.8] mb-10">
          ...
          <strong className="text-white font-medium">5+ years</strong> crafting
